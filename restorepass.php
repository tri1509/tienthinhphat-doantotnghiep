<?php
  ...
  if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['restorepass'])) {
    if(empty($_POST['passwordrestore'])) {
      $error['passwordrestore'] = "Không được để trống mật khẩu";
    }else{
      $pattern = '/^[A-Za-z0-9_\.!@#$%^&*()]{6,32}$/';
      if(!preg_match($pattern,$_POST['passwordrestore'])){
        $error['passwordrestore'] = "Bạn vui lòng đặt mật khẩu phức tạp hơn";
      }else{
        $password = md5($_POST['passwordrestore']);
      }
    }
    if(empty($error)){
      $data = array(
      ...
      $restorepass = $cs->restore_Pass($data,$id);
    }
  }
?>
                        ...
                        <form action="" method="post">
                          ...
                          <div class="form-label-group position-relative">
                            ...
                            <span style="color:red;font-size:12px">
                              <?php
                                if(!empty($error['passwordrestore'])) {
                                  echo $error['passwordrestore'];
                                }else{echo "<div class='clear20'></div>";} 
                              ?>
                            </span>
