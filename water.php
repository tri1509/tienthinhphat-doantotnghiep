<?php
	...
	$brand = new brand();
	$namebywater = $brand -> get_name_by_water($id);
	if($namebywater) {
		while($get_title = $namebywater -> fetch_assoc()){
		$title = $get_title['water_name'];
	}}else{
        ...
        <div class="row">
          <div class="col-12">
            <?php
              $namebywater = $brand -> get_name_by_water($id);
              if($namebywater) {
              $result_name = $namebywater -> fetch_assoc();
            ?>
            <h4 class="nomargin text-uppercase clredt">Nước giải khát
              : <?php echo $result_name['water_name'] ?>
            </h4><br><?php } ?>
            <div class="clear20"></div>
            <div class="row flex-wrap list-spc">
              <?php
                $productbywater = $brand -> get_product_by_water($id);
                if($productbywater) {
                  while($result = $productbywater -> fetch_assoc()) {
              ?>
